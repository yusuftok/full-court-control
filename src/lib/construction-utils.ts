...
export function getConstructionMessage(type: keyof typeof constructionMessages): string {
...
export function getConstructionPlaceholder(type: keyof typeof constructionPlaceholders): string {
...
export const constructionValidationMessages = {
  ...
  email: "That email address doesn't look quite right - double-check the blueprints? 📧",
  minLength: "We need a bit more detail here - like adding extra support beams! 🔨",
  maxLength: "Whoa there! That's more detail than we need - keep it blueprint-sized! 🗺️",
  number: "Numbers only in this field - just like measuring lumber! 📏",
  positive: "This needs to be a positive number - no negative vibes on the job site! ✨",
  password: "Make that password strong as reinforced concrete! 🔒"
}

// Progress calculation utilities
export function calculateConstructionProgress(completed: number, total: number): {
  percentage: number
  message: string
  emoji: string
} {
