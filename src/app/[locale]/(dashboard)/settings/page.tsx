...
export default function SettingsPage() {
  const breadcrumbItems = [{ label: 'Ayarlar', href: '/settings' }]
                ...
                >
                  <CardHeader className="pb-3">
                    <div className="flex items-center gap-3">
                      <div
                        className={`size-10 bg-gray-100 rounded-xl flex items-center justify-center`}
                      >
                        <Icon className={`size-5 ${colors.icon}`} />
                      </div>
                      <div className="flex-1">
                        <CardTitle
                          className={`text-heading-md ${colors.text} flex items-center gap-2`}
                        >
                          {setting.title}
              ...
              <Link key={setting.title} href={setting.href}>
                ...
                >
                  <CardHeader className="pb-3">
                    <div className="flex items-center gap-3">
                      <div
                        className={`size-10 bg-gradient-to-br from-${setting.color}-100 to-${setting.color}-200 rounded-xl flex items-center justify-center shadow-lg group-hover:shadow-xl transition-all duration-300 group-hover:scale-110`}
                      >
                        <Icon className={`size-5 ${colors.icon}`} />
                      </div>
                      <div className="flex-1">
                        <CardTitle
                          className={`text-heading-md ${colors.text} group-hover:text-primary transition-colors`}
                        >
                          {setting.title}
                        </CardTitle>
                      </div>
                      <div
                        className={`size-6 ${colors.icon} opacity-60 group-hover:translate-x-1 transition-transform`}
                      >
                        →
                      </div>
                    </div>
