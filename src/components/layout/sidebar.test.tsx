...
describe('Sidebar Component', () => {
  ...
  describe('Desktop Sidebar', () => {
    it('renders desktop sidebar with all navigation items', () => {
      ...
      const expectedItems = [
        'Dashboard', 'Projects', 'Division Templates', 'Tasks',
        'Subcontractors', 'Analytics', 'Reports', 'Settings'
      ]
      ...
      // Check footer
      expect(screen.getByText('© 2024 Full Court Control Pro')).toBeInTheDocument()
    })
  ...
  describe('Mobile Sidebar', () => {
    ...
    it('renders mobile sidebar when open', () => {
      ...
      const mobileSidebar = screen.getByText('Full Court Control').closest('div')
    ...
    it('hides mobile sidebar when closed', () => {
      ...
      const mobileSidebar = screen.getByText('Full Court Control').closest('div')?.parentElement
  ...
  describe('Brand Section', () => {
    ...
    it('renders brand text correctly', () => {
      ...
      expect(screen.getByText('Full Court Control')).toHaveClass('font-semibold', 'text-sm')
      expect(screen.getByText('Pro')).toHaveClass('text-xs', 'text-muted-foreground')
    })
  })
  ...
  describe('Navigation Items', () => {
    it('renders all navigation icons', () => {
      ...
      const expectedIcons = [
        'layoutdashboard', 'building2', 'foldertree', 'listtodo',
    ...
    it('applies hover states correctly', () => {
      ...
      const projectsLink = screen.getByRole('link', { name: /projects/i })
      expect(projectsLink).toHaveClass('hover:bg-accent', 'hover:text-accent-foreground')
    })
    ...
    it('applies focus states correctly', () => {
      ...
      const projectsLink = screen.getByRole('link', { name: /projects/i })
      expect(projectsLink).toHaveClass('focus-visible:outline-none', 'focus-visible:ring-2')
    })
  })
  ...
  describe('Responsive Behavior', () => {
    it('applies correct desktop classes', () => {
      ...
      const sidebar = screen.getByText('Full Court Control').closest('div')?.parentElement
    ...
    it('applies correct mobile classes when open', () => {
      ...
      const sidebar = screen.getByText('Full Court Control').closest('div')?.parentElement
      expect(sidebar).toHaveClass('fixed', 'inset-y-0', 'left-0', 'z-50', 'w-72')
    })
    ...
    it('applies transition classes for mobile', () => {
      ...
      const sidebar = screen.getByText('Full Court Control').closest('div')?.parentElement
      expect(sidebar).toHaveClass('transition-transform', 'duration-200', 'ease-in-out')
    })
  })
  ...
  describe('Custom ClassName', () => {
    ...
    it('applies custom className for mobile sidebar', () => {
      render(<Sidebar isMobile={true} isOpen={true} onClose={jest.fn()} className="custom-mobile" />)
...
describe('MobileMenuButton Component', () => {
  ...
  it('applies custom className', () => {
    render(<MobileMenuButton onClick={mockOnClick} className="custom-mobile-btn" />)
