...
async function processImageCheckIn(message: WhatsAppMessage) {
  ...
  if (!user) {
    await sendWhatsAppMessage(phoneNumber, "Please register your phone number with your account first.")
    return
  }
  ...
  const taskId = extractTaskIdFromMessage(message.caption || message.text?.body || "")
  ...
  await sendWhatsAppMessage(phoneNumber, "✅ Progress update received! Your supervisor will review shortly.")
}
```
