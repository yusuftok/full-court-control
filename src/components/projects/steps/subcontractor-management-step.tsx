...
export const SubcontractorManagementStep: React.FC<
  SubcontractorManagementStepProps
> = ({ formData, updateFormData }) => {
  const [newSubcontractorDialogOpen, setNewSubcontractorDialogOpen] =
    useState(false)
  const [newSubcontractorForm, setNewSubcontractorForm] =
    useState<NewSubcontractorForm>(emptyNewSubcontractorForm)
  const [selectedSubcontractorType, setSelectedSubcontractorType] =
    useState<SubcontractorType | null>(null)

  // Global subcontractor list (simulated - in real app would be from state management)
  const [subcontractorList, setSubcontractorList] =
    useState<Subcontractor[]>(mockSubcontractors)

  const handleSubcontractorChange = (
    type: 'constructionId' | 'mechanicalId' | 'electricalId',
    value: string | null
  ) => {
    updateFormData({
      subcontractors: {
  ...
  const handleAddNewSubcontractor = () => {
    ...
    handleSubcontractorChange(
      fieldMap[newSubcontractor.type],
      newSubcontractor.id
    )
  ...
  const completedSelections = Object.values(formData.subcontractors).filter(
    Boolean
  ).length
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Her iş türü için bir taşeron seçin. Mevcut listeden seçebilir veya
            yeni taşeron ekleyebilirsiniz.
          </CardDescription>
        </CardHeader>
        ...
        <CardContent className="pt-8">
          <div className="space-y-8">
            {subcontractorSections.map(section => {
              ...
              const selectedSubcontractor = selectedId
                ? findSubcontractorById(selectedId)
                : null
              const availableSubcontractors = getSubcontractorsByType(
                section.type
              )
              ...
              return (
                <Card
                  key={section.key}
                  className="border-l-4 border-l-slate-300 dark:border-l-slate-600"
                >
                  <CardContent className="pt-6">
                    <div className="grid grid-cols-12 gap-4 items-start">
                      <div
                        className={`w-12 h-12 rounded-lg bg-slate-100 dark:bg-slate-800 flex items-center justify-center ${section.color}`}
                      >
                      ...
                      <div className="col-span-6 flex flex-col">
                        <div className="flex-1 flex items-end">
                          <SubcontractorSelector
                            ...
                            onNewSubcontractor={() =>
                              openAddSubcontractorDialog(section.type)
                            }
      ...
      <Dialog
        open={newSubcontractorDialogOpen}
        onOpenChange={setNewSubcontractorDialogOpen}
      >
        <DialogContent className="sm:max-w-[500px] p-6">
          <DialogHeader className="px-0">
            <DialogTitle>
              ...
              {selectedSubcontractorType &&
                getSubcontractorTypeLabel(selectedSubcontractorType)}{' '}
              Taşeronu Ekle
            </DialogTitle>
            <DialogDescription>
              Yeni taşeron bilgilerini girin. Kaydedildikten sonra otomatik
              olarak seçili hale gelecek.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter className="pt-6">
            ...
            <button
              ...
              disabled={
                !newSubcontractorForm.companyName ||
                !newSubcontractorForm.responsiblePerson ||
                !newSubcontractorForm.phone
              }
          ...
          <CardContent className="pt-6">
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm text-amber-700 dark:text-amber-300">
                Devam edebilmek için tüm taşeron türleri seçilmelidir. Eksik:{' '}
                {totalSelections - completedSelections} taşeron
              </p>
            </div>
          </CardContent>
        </Card>
