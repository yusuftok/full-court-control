...
jest.mock('lucide-react', () => {
  ...
  return {
    ...originalModule,
    Plus: ({ className, ...props }: any) => <svg data-testid="plus" className={className} {...props} />,
    Search: ({ className, ...props }: any) => <svg data-testid="search" className={className} {...props} />,
    FolderTree: ({ className, ...props }: any) => <svg data-testid="folder-tree" className={className} {...props} />,
    Copy: ({ className, ...props }: any) => <svg data-testid="copy" className={className} {...props} />,
    Edit: ({ className, ...props }: any) => <svg data-testid="edit" className={className} {...props} />,
    Trash2: ({ className, ...props }: any) => <svg data-testid="trash2" className={className} {...props} />,
    ChevronRight: ({ className, ...props }: any) => <svg data-testid="chevron-right" className={className} {...props} />,
    ...
    Save: ({ className, ...props }: any) => <svg data-testid="save" className={className} {...props} />,
    X: ({ className, ...props }: any) => <svg data-testid="x" className={className} {...props} />,
  }
})
...
describe('DivisionTemplatesPage', () => {
  ...
  describe('Temel Render Testleri', () => {
    it('sayfa doğru bir şekilde render edilmelidir', () => {
      ...
      expect(screen.getByText('Projeleriniz için yeniden kullanılabilir bölüm hiyerarşileri oluşturun ve yönetin')).toBeInTheDocument()
    ...
    it('başlangıçta şablon kartları gösterilmelidir', () => {
      ...
      expect(screen.getByText('10 kattan yüksek konut kuleleri için standart bölüm şablonu')).toBeInTheDocument()
      expect(screen.getByText('Ahmet Yılmaz tarafından oluşturuldu')).toBeInTheDocument()
    ...
    it('boş durum mesajı searchTerm varken ve sonuç yokken gösterilmelidir', async () => {
      ...
      expect(screen.getByText('Aradığınızı bulmak için arama kriterlerinizi ayarlamayı deneyin.')).toBeInTheDocument()
    })
  })
  ...
  describe('State Yönetimi Testleri', () => {
    it('searchTerm state doğru çalışmalıdır', async () => {
      ...
      expect(screen.queryByText('Ticari Ofis Kompleksi')).not.toBeInTheDocument()
  ...
  describe('Şablon Seçimi ve Editor Testleri', () => {
    it('şablon düzenle butonu editor açmalıdır', async () => {
      ...
      expect(screen.getByText(/İpucu: Bölüm adlarını düzenlemek için çift tıklayın/)).toBeInTheDocument()
    })
    ...
    it('kaydet butonu alert göstermelidir', async () => {
      ...
      expect(mockAlert).toHaveBeenCalledWith(
        expect.stringContaining('✅ Yüksek Kat Konut Binası şablonu güncellendi!')
      )
    })
  })
  ...
  describe('Modal Testleri', () => {
    it('yeni şablon modal açılıp kapanmalıdır', async () => {
      ...
      expect(screen.getByText('İnşaat projeleriniz için yeniden kullanılabilir bölüm yapısı oluşturun.')).toBeInTheDocument()
      ...
      // Modal kapanmalı
      await waitFor(() => {
        expect(screen.queryByText('Yeni Şablon Oluştur')).not.toBeInTheDocument()
      })
    })
    ...
    it('boş form submit edildiğinde alert gösterilmelidir', async () => {
      ...
      // Alert çağrılmalı
      expect(mockAlert).toHaveBeenCalledWith('⚠️ Lütfen şablon adı ve açıklamasını girin')
    })
    ...
    it('projeye uygula modal açılıp kapanmalıdır', async () => {
      ...
      expect(screen.getByText(/"Yüksek Kat Konut Binası" şablonunu hangi projeye uygulamak istiyorsunuz?/)).toBeInTheDocument()
      
      // Projeler gösterilmeli
      expect(screen.getByText('Şehir Merkezi Ofis Kompleksi')).toBeInTheDocument()
      ...
      // Modal kapanmalı
      await waitFor(() => {
        expect(screen.queryByText('Şablonu Projeye Uygula')).not.toBeInTheDocument()
      })
    })
  })
  ...
  describe('Template Action Testleri', () => {
    it('template kopyalama işlevi çalışmalıdır', async () => {
      ...
      // Alert çağrılmalı
      expect(mockAlert).toHaveBeenCalledWith(
        expect.stringContaining('✅ "Yüksek Kat Konut Binası (Kopya)" oluşturuldu!')
      )
    })
    ...
    it('template silme işlevi çalışmalıdır', async () => {
      ...
      expect(mockConfirm).toHaveBeenCalledWith(
        expect.stringContaining('⚠️ Yüksek Kat Konut Binası şablonunu silmek istediğinizden emin misiniz?')
      )
      
      // Alert çağrılmalı
      expect(mockAlert).toHaveBeenCalledWith('🗑️ Yüksek Kat Konut Binası şablonu başarıyla silindi')
    })
