...
export default function TemplatesPage() {
  ...
  const templatesByCategory = React.useMemo(() => {
    return mockTemplates.reduce((acc, template) => {
      // Template kategori belirleme (basit heuristic)
      const category = 
        template.name.toLowerCase().includes('konut') ? 'residential' :
        template.name.toLowerCase().includes('ofis') || template.name.toLowerCase().includes('ticari') ? 'commercial' :
        template.name.toLowerCase().includes('köprü') || template.name.toLowerCase().includes('altyapı') ? 'infrastructure' :
        'renovation'
      ...
      <PageContent>
        ...
        <div className="space-y-8">
          {Object.entries(templatesByCategory).map(([categoryKey, templates]) => {
            const config = templateCategoryConfig[categoryKey as keyof typeof templateCategoryConfig]
            const Icon = config.icon

            return (
              <div key={categoryKey} className="space-y-4">
                <div className="flex items-center gap-3">
                  <div className={`size-8 ${config.bg} rounded-lg flex items-center justify-center`}>
                    <Icon className={`size-5 ${config.color}`} />
                  </div>
                  <h2 className="text-xl font-semibold">{config.label} Şablonları</h2>
                    ...
                    >
                      <CardHeader className="pb-3">
                        <div className="flex items-start justify-between">
                          ...
                          <div className={`size-10 ${config.bg} rounded-lg flex items-center justify-center flex-shrink-0`}>
                            <Icon className={`size-5 ${config.color}`} />
                          </div>
                        </div>
                      </CardHeader>
