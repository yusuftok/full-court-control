...
interface AnalyticsMetricCardProps extends Omit<StatCardProps, 'value' | 'change'> {
  primary: number
  secondary?: number
  tertiary?: number
  labels: {
...
export function AnalyticsMetricCard({
  ...
  const secondaryPercentage = total > 0 && secondary ? Math.round((secondary / total) * 100) : 0
  const tertiaryPercentage = total > 0 && tertiary ? Math.round((tertiary / total) * 100) : 0
        ...
        <div className="h-2 bg-secondary rounded-full overflow-hidden relative">
          <div 
            className={cn(
              ...
              status === 'success' ? 'bg-gradient-to-r from-green-500 to-green-400' :
      ...
      <div className="grid grid-cols-3 gap-2">
        <div 
          className={cn(
            ...
            status === 'critical' ? 'border-red-200 bg-gradient-to-br from-red-50 to-red-100' :
            status === 'warning' ? 'border-yellow-200 bg-gradient-to-br from-yellow-50 to-yellow-100' :
        ...
        >
          ...
          <div className="relative">
            ...
            <div className="text-xs text-muted-foreground truncate">{labels.primary}</div>
          </div>
        </div>
        ...
        {secondary !== undefined && (
          <div 
            ...
            onClick={interactive ? () => onSegmentClick?.('secondary') : undefined}
          >
            {interactive && (
              <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-500" />
            )}
            <div className="relative">
              ...
              <div className="text-xs text-muted-foreground truncate">{labels.secondary}</div>
            </div>
          </div>
        )}
        ...
        {tertiary !== undefined && (
          <div 
            ...
            onClick={interactive ? () => onSegmentClick?.('tertiary') : undefined}
          >
            {interactive && (
              <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-500" />
            )}
            <div className="relative">
              ...
              <div className="text-xs text-muted-foreground truncate">{labels.tertiary}</div>
            </div>
          </div>
        )}
      </div>
...
export function ProgressStatCard({
  ...
  const [justReachedMilestone, setJustReachedMilestone] = React.useState<number | null>(null)
  
  // Check for milestone achievements
  React.useEffect(() => {
    const milestones = [25, 50, 75, 100]
    const reachedMilestone = milestones.find(m => 
      percentage >= m && percentage < m + 5 // Recently reached
    )
            ...
            <div className={cn(
              ...
              <div 
                className={cn(
                  ...
                  percentage >= 100 ? "gradient-success" :
