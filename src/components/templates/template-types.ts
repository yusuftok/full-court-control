...
export interface InteractiveDivisionTreeProps {
  ...
  onNodeMove?: (draggedNodeId: string, targetNodeId: string, position: 'inside' | 'before' | 'after') => void
  selectedNodeId?: string
  editingNodeId?: string
  onKeyboardNavigation?: (nodeId: string, direction: 'up' | 'down' | 'left' | 'right') => void
  // Global drag state
  draggedNode?: string | null
  dragOverNode?: string | null
  dropPosition?: 'inside' | 'before' | 'after' | null
  onDragStateChange?: (draggedNode: string | null, dragOverNode: string | null, dropPosition: 'inside' | 'before' | 'after' | null) => void
}
